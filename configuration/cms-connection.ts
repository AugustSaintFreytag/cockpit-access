...
import { NuxtProcess } from "../modules/cockpit/library/nuxt-process"
...
export class CmsConnection implements Configuration.Connection {
	...
	private requestedContext(context?: Configuration.Context) {
		if (context === undefined) {
			return this.isRenderingServerSide() ? Configuration.Context.Server : Configuration.Context.Client
		}

		return context
	}

	private isRenderingServerSide(): boolean {
		return (process as NuxtProcess).server || false
	}
